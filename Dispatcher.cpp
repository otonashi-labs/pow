...
static void printResult(cl_ulong4 seed, cl_ulong round, result r, cl_uchar score, const std::chrono::time_point<std::chrono::steady_clock> & timeStart, const Mode & mode, bool verboseStdOut) {
	// Time delta
...
void Dispatcher::handleResult(Device & d) {
    for (auto i = PROFANITY_MAX_SCORE; i > 0; --i) { // Changed from i > m_clScoreMax to i > 0
        ...
        if (r.found > 0) {
            ...
            printResult(d.m_clSeed, d.m_round, r, i, timeStart, m_mode, this->verboseStdOut);
            break;
        }
    }
}
...
// needs to be disabled on a multiple gpus
